...
def test_basic_distribution_sums_to_one():
    ctx = MoveContext(
        archetype="parasite", last_3_moves=[], tension_level=5,
        momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
...
def test_counter_loop_breaker():
    ctx = MoveContext(
        archetype="parasite", last_3_moves=["COUNTER", "COUNTER", "COUNTER"],
        tension_level=5, momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=3, consecutive_low_scores=0
    )
    dist = compute_distribution(ctx)
...
def test_signature_move_capped_at_040():
    for archetype, sig_move in ARCHETYPE_SIGNATURE_MOVE.items():
        ctx = MoveContext(
            archetype=archetype, last_3_moves=[], tension_level=None,
            momentum=None, arc_theme="power", fear_keywords=[],
            consecutive_counters_in_pair=0, consecutive_low_scores=0
        )
        dist = compute_distribution(ctx)
...
def test_non_signature_at_least_002():
    for archetype, sig_move in ARCHETYPE_SIGNATURE_MOVE.items():
        ...
        for move, prob in dist.probabilities.items():
            if move != sig_move:
                assert prob >= 0.02 - 0.001, (
                    f"{archetype}: {move} = {prob} < 0.02"
                )
...
def test_consecutive_move_penalty():
    ...
    ctx = MoveContext(
        archetype="prophet", last_3_moves=["QUESTION", "QUESTION"],
        tension_level=None, momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
    dist = compute_distribution(ctx)
    base_ctx = MoveContext(
        archetype="prophet", last_3_moves=[], tension_level=None,
        momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
...
def test_losing_the_room_pivot_dominant():
    ctx = MoveContext(
        archetype="prophet", last_3_moves=[], tension_level=None,
        momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=2
    )
    dist = compute_distribution(ctx)
...
def test_high_tension_boosts_concede_pivot():
    # Without tension
    ctx_low = MoveContext(
        archetype="trickster", last_3_moves=[], tension_level=5,
        momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
    ...
    ctx_high = MoveContext(
        archetype="trickster", last_3_moves=[], tension_level=9,
        momentum=None, arc_theme="power", fear_keywords=[],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
...
def test_fear_keyword_match_boosts_escalate_question():
    ...
    ctx_no_fear = MoveContext(
        archetype="parasite", last_3_moves=[], tension_level=None,
        momentum=None, arc_theme="power", fear_keywords=["exposure", "worthlessness"],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
    ...
    ctx_fear = MoveContext(
        archetype="parasite", last_3_moves=[], tension_level=None,
        momentum=None, arc_theme="exposure", fear_keywords=["exposure", "worthlessness"],
        consecutive_counters_in_pair=0, consecutive_low_scores=0
    )
...
def test_all_archetypes_produce_valid_distributions():
    for archetype in ARCHETYPE_SIGNATURE_MOVE:
        ctx = MoveContext(
            archetype=archetype, last_3_moves=[], tension_level=5,
            momentum="escalating", arc_theme="power", fear_keywords=[],
            consecutive_counters_in_pair=0, consecutive_low_scores=0
        )
        dist = compute_distribution(ctx)
        total = sum(dist.probabilities.values())
