...
async def test_high_tension_uses_visceral_markers(
    ...
    assert has_visceral or has_modifier, f"Expected visceral language at tension=8, got: {result}"
...
async def test_reconciliation_mixed_feelings(
    ...
    assert has_reconciliation or has_framing, f"Expected reconciliation language, got: {result}"
...
async def test_output_present_tense_only(
    ...
    for archetype in ["parasite", "prophet", "martyr", "keeper"]:
        for tension in [2, 5, 8]:
            ...
            assert not re.search(r"\b1[67]\d{8}\b", result), (
    ...
    for i, arch_a in enumerate(archetypes):
        for arch_b in archetypes[i + 1 :]:
            assert results[arch_a] != results[arch_b], (
                f"Archetypes {arch_a} and {arch_b} produced identical output: {results[arch_a]}"
            )
...
async def test_output_within_token_budget(
    ...
    assert token_count <= 200, f"Token count {token_count} exceeds budget of 200. Output: {result}"
