...
For any input where the bug condition holds, the fixed VoiceSurface.compose()
SHALL produce a valid VoicePlan without blocking for more than 200ms, SHALL log
any errors or degraded states at WARNING level or higher, and SHALL not silently
suppress TTS output.
"""
...
def _stale_snapshot_no_headline(dialogue_age: int = 25) -> dict[str, Any]:
    """Snapshot with stale dialogue and NO showrunner headline/audience_prompt."""
    return {
        "epoch": 1000,
        "showrunner": {
            "scene": "ensemble-stage",
            "speaker": "Alpha",
            # No headline, no audience_prompt — forces static fallback
        },
        "audience": {"patronage_index": 14.0},
        "broadcast": {
            "caption": {},
        ...
        "last_dialogue_turn": {
            ...
            "sender_name": "Elder-Hook-6A4A",
            "sent_at": 1000 - dialogue_age,  # age > 20s threshold
        },
    }


# ---------------------------------------------------------------------------
# Test 1: Synchronous blocking — compose() must NOT block on slow probes
...
class TestSwallowedException:
    ...
    def test_build_voice_state_logs_warning_on_exception(self, caplog):
        ...
        making failures invisible to operators.
        """
        snapshot = _base_snapshot()
...
class TestAbruptFallback:
    ...
    def test_stale_dialogue_no_static_fallback(self):
        """Assert output is NOT the static string when dialogue is stale with no headline.
    ...
    def test_stale_dialogue_graceful_fallback_property(self, dialogue_age):
        ...
        **Validates: Requirements 1.3**
        ...
        snapshot = _stale_snapshot_no_headline(dialogue_age=dialogue_age)
        surface = VoiceSurface(enabled=True, dry_run=False)
...
class TestNoRetry:
    ...
    def test_health_probe_retries_after_failure(self):
        ...
        leaving voice marked unhealthy for the entire snapshot cycle (~30s).
        """
        call_count = {"n": 0}
    ...
    def test_probe_retry_recovers_property(self, failures_before_success):
        """Property: After 1 transient failure (within max_retries), health recovers.
