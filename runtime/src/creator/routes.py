...
VALID_PETITION_TYPES = {
    "domain", "llc", "stripe", "google_workspace", "linkedin", "x_account",
    "compute_node", "custom_integration", "mercy", "code_audit",
}
...
async def resolve_petition(petition_id: str, body: dict):
    ...
    if action not in ("approve", "reject", "counter"):
        return JSONResponse(status_code=422, content={"error": "action must be approve|reject|counter"})
    ...
    if action == "approve":
        ...
        cur.execute(
            ...
            (new_status, now, body.get("creator_notes", ""), body.get("result_summary", ""), petition_id),
        )
        event_narrative = (
        ...
        cur.execute(
            "UPDATE creator_petitions SET status = %s, resolved_at = %s, "
            "creator_notes = %s WHERE petition_id = %s",
            (new_status, now, body.get("creator_notes", f"Counter-offer: ${counter_fee:.2f}"), petition_id),
        )
        event_narrative = (
    ...
    emitter = await get_emitter()
    await emitter.emit("creator", f"petition.{action}d", {
        "petition_id": petition_id,
