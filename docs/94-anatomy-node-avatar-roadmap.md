...
| M05 | complete | LLM anatomy control contract. | LLM gets only valid handles through an Ollama structured-output schema; invented nodes and unsupported capabilities are rejected with diagnostics; accepted controls retain source-backed node ids; local `llama3.1:8b` path was tested; browser evidence exists at `artifacts/m05-anatomy-control-contract.png` with timed-frame diff evidence. |
| M06 | complete | Browser anatomy inspection renderer. | Browser renders graph-derived body/system/head/knee/toe layers from `render_projection`; screenshots prove visible output; missing mappings degrade visibly with renderer diagnostics; evidence exists at `artifacts/m06-anatomy-inspection-renderer.png` with timed-frame diff evidence. |
| M07 | complete | Motion projection bridge. | Anatomy action bundles map to browser renderer controls plus OpenSim/Moco/MuSkeMo-ready hints; wave/sit/run have deterministic compiled plans; missing shoulder/elbow/hip/pelvis/ankle depth degrades explicitly; browser evidence exists at `artifacts/m07-anatomy-motion-bridge.png` with timed-frame diff evidence. |
| M08 | next | Right-hand digit graph correction. | Thumb through little finger are modeled as part of the right hand and right upper limb; all five metacarpals, all fourteen hand phalanges, and CMC/MCP/IP/PIP/DIP joint nodes connect the chains; the browser renders the graph-backed hand rays and tests prove no fake phalanx or joint is accepted. |
