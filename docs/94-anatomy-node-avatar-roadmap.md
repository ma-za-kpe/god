...
| M04 | next | GraphRAG retrieval and LOD compiler. | Queries such as `wave`, `run`, and `sweat on forehead` compile into bounded working sets; no million-node context dumps; tests prove LOD behavior. |
| M05 | pending | LLM anatomy control contract. | LLM gets only valid handles; invented nodes are rejected; action plans include diagnostics and source-backed node ids; Ollama/local model path tested. |
